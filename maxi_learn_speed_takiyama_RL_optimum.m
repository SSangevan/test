function [] = maxi_learn_speed_takiyama_RL_optimum()
...
for j = 1:T
    ...
    for i = 1:nInput
        cost(i)=yopti(i)-y(i);
        r(i)= max(0,(rewardThreshold - abs(cost(i)))/rewardThreshold);
        %E(i)=cost;
    end
    ...
    for i = 1:nInput
        deltaW(i) = (r(i)-expectR(i))*((A(i)-mu(i))/omega(i));
        deltaV(i) = (r(i)-expectR(i));
    ...
    X(:,j)=xOutput;
    P=transpose(cost)*cost;
    E(j)=P;
    E2(j)=(1/2)*(tTarget-xOutput)'*(tTarget-xOutput);
end
%xOutput=R*Z*A
